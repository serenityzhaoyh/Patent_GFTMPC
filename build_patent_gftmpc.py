...
def build_doc(fig1, fig2):
    ...
    doc.title("权利要求书")
    doc.P(
        "1. 一种基于GFTMPC的柔性钻孔控制方法，其特征在于，包括如下步骤："
        "采集钻孔过程中的钻孔角度、进给速度和进给扭矩数据，并对所述数据进行滤波预处理；"
        "基于不同钻孔角度下的历史钻孔数据构建多角度参考扭矩库；"
        "以钻孔角度、历史进给速度和历史进给扭矩作为输入，构建基于GRU神经网络的进给扭矩预测模型GFTM，以预测下一控制时刻的进给扭矩；"
        "将参考扭矩、预测扭矩、进给速度约束和进给速度增量约束引入模型预测控制器，建立以扭矩跟踪误差和控制增量为目标的滚动优化问题；"
        "在钻孔执行过程中实时输出优化后的进给速度并作用于钻孔加速阶段和稳定钻孔阶段，形成进给速度闭环调节；"
        "当外部干扰、切屑堵塞趋势或姿态变化导致进给扭矩偏离参考轨迹时，依据预测结果主动调整进给速度，使进给扭矩回归参考轨迹附近。"
    )
    doc.P(
    ...
    doc.P(
        "8. 一种基于GFTMPC的柔性钻孔控制系统，其特征在于，包括数据采集模块、参考扭矩库模块、"
        "GFTM进给扭矩预测模块、模型预测控制模块以及执行控制模块；"
        "所述数据采集模块用于采集钻孔角度、进给速度和进给扭矩数据；"
        "所述参考扭矩库模块用于构建并输出与当前钻孔角度匹配的参考扭矩轨迹；"
        "所述GFTM进给扭矩预测模块用于根据钻孔角度、历史进给速度和历史进给扭矩预测下一控制时刻的进给扭矩；"
        "所述模型预测控制模块用于根据参考扭矩、预测扭矩及约束条件求解优化后的进给速度；"
        "所述执行控制模块用于将优化后的进给速度写入钻孔执行机构，以完成柔性钻孔控制。"
    )
    doc.P(
    ...
    doc.P(
        ...
        "利用基于GRU神经网络的GFTM模型预测下一控制时刻的进给扭矩；"
